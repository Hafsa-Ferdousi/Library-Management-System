...
void issueBook() {
    ...
    while (fread(&b, sizeof(b), 1, fp) == 1) {
        if (b.id == s.id) {
            ...
            s.bookName[sizeof(s.bookName) - 1] = '\0';  // Ensure null-terminated string
            f = 1;
            break;
        }
    }
...
void returnBook() {
    ...
    fp = fopen("issue.txt", "rb");
    FILE *ft = fopen("tmp_issue.txt", "wb");

    while (fread(&s, sizeof(s), 1, fp) == 1) {
        if (id == s.id) {
            // Book found in issued list, mark it as returned
            strncpy(s.bookName, "Returned ", sizeof(s.bookName) - 1);
    ...
    if (f == 1) {
        ...
        printf("\nNo Book Found in Issued List.\n");
    }

    remove("issue.txt");
    rename("tmp_issue.txt", "issue.txt");
}
